...
//				result += '(' + proc.paralist.para.get(i).type + ') para.get(' + i + ')'
				result +=  genStringConvertFunction(proc.paralist.para.get(i).type.toString(), 'para.get(' + i + ').trim()')
	...
	static def String genStringConvertFunction(String type, String param) {
		if (type == 'int') return 'Integer.parseInt(' + param + ')'
		if (type == 'byte') return 'Byte.parseByte(' + param + ')'
		return param + '.trim()'
	}
