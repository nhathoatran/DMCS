...
«««			«ENDFOR»
	...
	'''			
		...
		if («Data.getPeriodicClockVariable(periodic)» == 0) { //period «Data.periodicClockProperties.get(Data.getPeriodicClockVariable(periodic))»		 	
			«IF (Data.periodicClockLimit.get(Data.getPeriodicClockVariable(periodic)) != null)»
				if («Data.getPeriodicClockVariable(periodic)»_count <= «Data.periodicClockLimit.get(Data.getPeriodicClockVariable(periodic))»){
					«Data.getPeriodicClockVariable(periodic)»_count ++;
