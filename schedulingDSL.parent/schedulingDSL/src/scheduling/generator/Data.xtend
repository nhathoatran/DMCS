...
class Data {
	...
	public static  HashMap<String, String> periodicClockLimit = new HashMap<String, String>() ;
		...
		if (procModel.processconfig != null) {
			for (proc : procModel.processconfig.procinit) {
				if (proc.periodic != null) {						
					...
					if (proc.periodic.max != 0)
						periodicClockLimit.put(getPeriodicClockVariable(proc.periodic), proc.periodic.max.toString)
				}				
			}
		}
	}
