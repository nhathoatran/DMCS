...
«««			«ENDIF»		
	...
	'''	
		...
		«FOR pClock : Data.periodicClockOffset.keySet()»
			...
			«ENDIF»
		«ENDFOR»
		«FOR pClock : Data.periodicClockLimit.keySet()»
			«IF pClock.length != 0»
				//limitation of the periodic process
				public int «pClock»_count = 0 ; //max : «Data.periodicClockLimit.get(pClock)»
			«ENDIF»
		«ENDFOR»
		«IF sch.schedulerdata != null»
...
«««		«ENDIF»						
		public void encode(DataWriter _writer) {					
			...
			«ENDFOR»
			«FOR pC : Data.periodicClockLimit.keySet()»
				_writer.writeInt(«pC»_count) ; // periodic process count
			«ENDFOR»						
...
«««			initial before								
		...
		public boolean decode(DataReader _reader) {				
			...
			«ENDFOR»	
			«FOR pC : Data.periodicClockLimit.keySet()»
				«pC»_count = _reader.readInt() ; // periodic process count
			«ENDFOR»						
