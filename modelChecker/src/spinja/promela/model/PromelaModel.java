...
//	}
	...
	public Channel[] endProcess() {
		...
		_procs[_nrProcs] = null; //need to keep process in model?
	...
	public Channel[] endProcess(int processID) {
		...
		_procs[processID] = null; //need to keep process in model? lost reference
		_nrProcs -- ;
	...
	public void updateProcessSize() {
		if (Config.isCompileScheduler || Config.isCompileSchedulerOptimize) {
			_process_size = 0;
			int i = 0 ;
			int index = 0 ;
			while (i < _nrProcs && index < 255) {
				if (_procs[index] != null && SchedulerObject.processInScheduler[index]) {
					_process_size += _procs[i].getSize();
					i++ ;
				}
				index++ ;
			}
	...
	public int countNumTrans(){ //for heuristic search
		int count= 0 ;		
		if (_nrProcs > 0) {			
			for (int i = 0 ; i < _nrProcs ; i++ ) {
				PromelaProcess proc = _procs[i] ;
				if (proc != null)
					count += proc.countNumTrans() ;
			}				
		}
		return count ;
	}
