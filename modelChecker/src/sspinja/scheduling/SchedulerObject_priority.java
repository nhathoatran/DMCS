...
public class SchedulerObject_priority {
	...
	public void init_order() throws ValidationException {
		...
		{
			int processID = getProcessID("P1") ;
			if (processID >= 0) { 
				//create new process in model
				//SchedulerPanModel.p
				//create new process information in scheduler
				SchedulerProcess P1 = new SchedulerProcess() ;
				//P1.processID = (byte) processID ;
				P1.processID = processID ;
				
				while (pcnt.size() < processID + 1) pcnt.add((byte) 0) ;				
				pcnt.set(processID, (byte) (pcnt.get(processID) + 1));
								
				P1.refID = getRefID("P1") ;										
				P1.P1() ;										
				//processList.set(processID, "P1_0") ;
				processList.set(processID, "P1") ;
	...
	public void config_new_process (SchedulerProcess target) throws ValidationException {		
		//defined by new_process event handler	
		//initProcessList = false - ?
		{//MoveProcess target			
			if (target != null) {
				remove_process(target.processID) ;
				ready.put(target) ;
					
				if (running_process != null) {
					if (running_process.processID == target.processID) {
						running_process = null;
					}
				}	
			}
		...
		if (!((running_process == null)))
			{//BlockStatement
				if (((target.get_priority())>(running_process.get_priority())))
					{//BlockStatement
						{//MoveProcess target			
							if (running_process != null) {
								ready.put(running_process) ;
									
								if (running_process != null) {
									if (running_process.processID == running_process.processID) {
										running_process = null;
									}
								}	
								//running_process = null ;
	...
	public void addProcessList(ArrayList<SchedulerProcess> procList) throws ValidationException { //called by init_order
		...
		for (SchedulerProcess target : procList) {	
				...
				{//BlockStatement
					if (((target.get_priority())>(running_process.get_priority())))
						{//BlockStatement
							{//MoveProcess target			
								//when initializing: put new process to temporary list (AL_?) for stack or queue, then put temporary list into stack or queue
								AL_ready.add(running_process) ;
							}
						}
				}
			
				//initProcessList = false
			initprocesslist.add(getInstance(target)) ;
		}
